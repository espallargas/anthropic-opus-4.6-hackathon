namespace :countries do
  desc 'Seed countries to the database (idempotent)'
  task seed: :environment do
    ...
    countries_data.each do |data|
      country = Country.find_by(code: data[:code])

      if country
        if country.update(data.slice(:name, :flag_emoji))
          updated_count += 1
        end
      else
        Country.create!(data)
    ...
    puts "✅ Seeding complete: #{created_count} created, #{updated_count} updated"
  end
end
